...
Before final commit, final push, PR creation or update, final review request, story `review` or
`done` status completion, or PR merge, the final readiness decision must be orchestrated by
`.claude/skills/pr-readiness-check/SKILL.md`.

If the branch is behind `origin/main`, parallel work is detected, or artifacts may be stale, run
this audit first. If it already ran for the same branch, commit, target, and freshly fetched
remote state, the PR readiness audit must reuse its result.
