...
The React application for the People Management Platform. Detailed per-area conventions live in `.claude/rules/` and load automatically when working with matching files.
...
- **Quality**: ESLint + Prettier — base configs shared via `libs/config/` (see
  `libs/config/README.md`); `eslint.config.js` spreads `libs/config/eslint-react.config.js`,
  `tsconfig.app.json` extends `libs/config/tsconfig.react.base.json`, `.prettierrc.cjs` extends
  `libs/config/prettier.base.json`
...
- The BFF is expected at `http://localhost:3001` (`VITE_API_BASE_URL`). Per
  ...
  this frontend talks **only** to the BFF, never directly to a domain service — no backend exists
  yet, but when one does, it's reached through the BFF's composed API, not called directly
