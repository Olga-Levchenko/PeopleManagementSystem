...
import type { IdentityResolutionPort } from './identity-resolution.port';
...
export class RequestActorContext {
  ...
  constructor(
    ...
    @Inject('IdentityResolutionPort')
    private readonly identityResolution: IdentityResolutionPort,
  ) {}
  ...
  private async resolveActorIdInternal(): Promise<string> {
    ...
    const result = await this.identityResolution.resolve(
      iss,
      sub,
      this.accessToken,
    );
