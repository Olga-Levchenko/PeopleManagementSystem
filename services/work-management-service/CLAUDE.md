...
Work Management — per [ARCHITECTURE-SPINE AD-1](../../_bmad-output/planning-artifacts/architecture/architecture-PeopleManagementSystem-2026-08-25/ARCHITECTURE-SPINE.md), owns risks, action items, CDS, mentorship, campaigns, and feedback. Per-area conventions live in `.claude/rules/` and load automatically when working with
matching files.
...
- `npm run start:dev` — dev server with watch (port 3004)
...
- App port **3004**; CORS is open for `http://localhost:4200` (the React frontend, via the BFF in
