...
- **Tests**: Jest — unit in `src/modules/*/__tests__/`, e2e in `test/` (`test/jwt-guard.e2e-spec.ts`
  ...
  already-running Postgres — do not run the full `test:e2e` suite without one)
...
- `modules/` — feature modules, one folder per feature; seeded modules today are `modules/health`,
  `modules/organisational-relationships`, `modules/outbox`, and `modules/auth` (the global JWT
  guard — `jwt.strategy.ts`, `jwt-auth.guard.ts`, `public.decorator.ts`, `auth.module.ts`) — add
