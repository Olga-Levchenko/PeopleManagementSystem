...
- `modules/` — feature modules, one folder per feature; seeded modules today are `modules/health`,
  ...
  section-gated S1/S2 profile response; see Gotchas for its audience-resolution contract. Spec-1-6b
  added PP-line as a third, independent qualifying line alongside Reporting-line/Project-line —
  see Gotchas) — add further feature modules under `modules/<name>`
...
- **`modules/profile`'s audience resolution fails closed at the adapter, not the service.**
  ...
  and returns the same `{reportingLine:false, projectLine:false, peoplePartnerLine:false,
  managerSectionAccess:null, peoplePartnerSectionAccess:null}` shape (`NEITHER_LINE_RESOLUTION`) a
  genuine Colleague resolution would produce — `ProfileService` never distinguishes "resolver said
  no access" from "resolver was unreachable" and never try/catches the call itself. Don't add error
  handling in `ProfileService` for this — the adapter's contract already makes it unreachable.
- **PP-line (spec-1-6b) is a third, independent qualifying line in `ProfileService.resolveAudience`**
  — checked after the existing Reporting-line/Project-line (`managerSectionAccess`) branch, which
  still takes priority if both qualify (the section-matrix cells are identical either way, so order
  has no visible effect on the result; Manager was simply checked first already). PP-line qualifies
  only when `peoplePartnerLine` is `true` AND `peoplePartnerSectionAccess` is non-null — same
  defensive pattern as the Manager check, failing closed to Colleague on a malformed/partial
