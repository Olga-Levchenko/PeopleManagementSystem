...
@ApiBearerAuth()
@Controller('internal/pp-dashboard')
export class PPDashboardMetadataController {
  constructor(
  ...
  async getPPDashboardMetadata() {
    const [callerPersonId, granted] = await Promise.all([
      ...
      this.checkViewDashboardPermission('people-partner'),
    ]);
    if (!granted) throw new ForbiddenException();
    return this.service.getPPDashboardMetadata(callerPersonId);
  }
