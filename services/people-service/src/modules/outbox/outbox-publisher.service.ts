...
export class OutboxPublisherService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    this.interval = setInterval(() => this.schedulePublish(), intervalMs);
  ...
  private schedulePublish(): void {
    this.publishPending().catch(() => {
      this.logger.error('Scheduled outbox publication failed');
    });
  }

  private async claimPending(): Promise<ClaimedOutboxEvent[]> {
    const staleLockMinutes = this.config.getOrThrow<number>(
      'OUTBOX_STALE_LOCK_MINUTES',
    );
  ...
  private async publishClaimed(event: ClaimedOutboxEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Confirmed event ${event.eventId} could not be marked published`,
        error,
      );
    }
  }
  ...
  private async recordFailure(
    ...
    const retryLimit = this.config.getOrThrow<number>(
      'OUTBOX_PUBLISHER_RETRY_LIMIT',
    );
  ...
  private backoffMs(retryCount: number): number {
    ...
    return (
      delaysInMinutes[Math.min(retryCount, delaysInMinutes.length) - 1] * 60_000
    );
  }
}
