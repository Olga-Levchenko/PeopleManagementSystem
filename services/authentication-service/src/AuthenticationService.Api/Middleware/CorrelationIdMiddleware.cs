...
/// <summary>
/// Mirrors the Node services' correlation-id convention (and <c>access-control-service</c>'s own
/// port of it): read <c>x-correlation-id</c>, generate a new id if it is absent/blank, echo the
/// resolved id on the response, and log method/url/id. A blank/whitespace-only incoming value is
/// treated as absent (never echoed back blank), and when the header repeats, the first non-empty
/// value is used (never comma-joined). A value containing control characters (e.g. CR/LF, which
/// could split a response header or corrupt a log line) or exceeding <see cref="MaxLength"/> is
/// likewise treated as absent, rather than echoed/logged verbatim.
/// </summary>
