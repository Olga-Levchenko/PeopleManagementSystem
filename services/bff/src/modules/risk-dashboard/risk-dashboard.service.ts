...
export interface RiskDashboardUpstreamResponse {
  status: number;
  body: unknown;
}
...
export class RiskDashboardService {
  ...
  ): Promise<RiskDashboardUpstreamResponse> {
    try {
      ...
      const metadataPeople: Array<{
        personId: string;
        fullName: string;
        department: { id: string; label: string } | null;
        projects: Array<{ id: string; label: string }>;
        manager: { id: string; label: string } | null;
        peoplePartner: { id: string; label: string } | null;
      }> = [];
      for (let index = 0; index < ids.length; index += 500) {
        const metadataResponse = await fetch(
          `${this.config.getOrThrow<string>('PEOPLE_SERVICE_URL')}/api/v1/internal/risk-dashboard/metadata`,
          {
            method: 'POST',
            headers: {
      ...
      const byId = new Map(
        metadataPeople.map((person) => [person.personId, person]),
      );
      const rows = dashboard.rows.flatMap((row) => {
        const person = byId.get(row.personId);
        if (!person) return [];
        if (query.departmentId && person.department?.id !== query.departmentId)
          return [];
        if (
          query.projectId &&
          !person.projects.some((project) => project.id === query.projectId)
        )
          return [];
        if (query.managerId && person.manager?.id !== query.managerId)
          return [];
        if (
          query.peoplePartnerId &&
          person.peoplePartner?.id !== query.peoplePartnerId
        )
          return [];
        return [{ ...row, ...person }];
      });
      ...
      const options = <T extends { id: string; label: string }>(
      ...
      if (!Number.isInteger(offset) || offset < 0)
        return { status: 400, body: { message: 'Request failed' } };
      const page = rows.slice(offset, offset + pageSize);
      const nextCursor =
        rows.length > offset + pageSize
      ...
      return {
        ...
        body: {
          ...
          nextCursor,
          catalogs: {
            departments: options(rows.map((row) => row.department)),
            projects: options(rows.flatMap((row) => row.projects)),
            managers: options(rows.map((row) => row.manager)),
            peoplePartners: options(rows.map((row) => row.peoplePartner)),
          },
        },
      };
    } catch {
