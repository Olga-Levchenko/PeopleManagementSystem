...
Resourcing — per [ARCHITECTURE-SPINE AD-1](../../_bmad-output/planning-artifacts/architecture/architecture-PeopleManagementSystem-2026-08-25/ARCHITECTURE-SPINE.md), owns requests, candidates, proposals, approvals, request history, and resourcing-specific workflow state. Per-area conventions live in `.claude/rules/` and load automatically when working with
matching files.
...
- App port **3003**; CORS is open for `http://localhost:4200` (the React frontend, via the BFF in
