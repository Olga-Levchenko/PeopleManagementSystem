...
| S3 | Emergency contacts | Contact person, relationship, phone | RW | R | **—** | RW | — | — (never shareable) | partial (Story 2.6b: Self S3 CRUD + profile GET; Story 2.10: Manager/PP read on profile GET — `profile-audience.util.spec` / `profile.service.spec` / `profile.e2e-spec`) |
...
| S5 | Documents | Contract, W8, cooperation form, Diia City, CV, certificates | R (own) + upload certificates | R | **R, CV + certificates only** | RW | — | cfg | partial (Story 2.6b: Self certificate upload + profile GET metadata; Story 2.10: Manager/PP read + cert download — `profile-audience.util.spec` / `profile.service.spec` / `profile.e2e-spec`) |
| S6 | Risks | Current level, trend, description, details, date, full history — no closed/terminal state (4.6) | — | RW | RW | RW | — | cfg | partial (Story 2.7: Self never receives `s6` — negative tests in `profile.service.spec` / `profile.e2e-spec`; Story 4.3: WMS profile risk read/append authorization tests plus BFF/frontend profile risk smoke) |
...
| S9 | Career timeline | System-generated event log (4.9); includes department change; departure/dismissal is explicitly NOT a timeline event (v1.5) — see S4 employment status instead | R | RW | RW | RW | — | cfg | partial (Story 2.7: Self read; Story 2.9: Manager/PP read on profile GET — `profile-audience.util.spec` / `profile.service.spec` / `profile.e2e-spec`) |
