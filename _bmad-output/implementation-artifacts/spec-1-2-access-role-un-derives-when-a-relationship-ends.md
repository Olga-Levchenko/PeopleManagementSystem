...
**Never:**
- No new caching layer, and no change to `AccessRoleResolver`/`EfRelationshipRepository`'s existing query logic — if a test fails, the fix is in the test's assumptions, not in adding caching or changing resolution behavior.
...
**Execution:**
- [x] `services/access-control-service/tests/AccessControlService.Api.Tests/AccessRoleResolverCompositionTests.cs` — add tests for the five I/O matrix scenarios above, all against the real DI-composed resolver — proves this story's core guarantee end-to-end

**Acceptance Criteria:**
- Given a reports-to or department-management relationship change made directly against `AccessControlDbContext` (standing in for Story 1.3's not-yet-built screen), when the next `ResolveAsync` call runs for the affected (viewer, subject) pair, then the result reflects the change with no propagation delay
- Given a project-assignment-ended event processed through `ProjectAssignmentEventProcessor`, when the resolution engine's next `ResolveAsync` call runs, then Project-line access derived solely from that assignment is absent
