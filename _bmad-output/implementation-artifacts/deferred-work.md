...
## Deferred from: code review of story-1-5 (2026-09-04)

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: Last-holder guard in `FullProfileAccessController.Revoke` is a TOCTOU race: `GetActiveCountAsync` and `RevokeAsync` are not atomic. Two concurrent revokes for different subjects when count=2 can both pass the `count <= 1` check and both proceed, leaving zero holders and violating the spec §2.4 invariant. Fix requires either (a) a `SELECT ... FOR UPDATE` / serializable transaction wrapping the count-check and removal together, or (b) a database-level constraint (e.g. a trigger or a `CHECK (count >= 1)` enforced at commit time).
  evidence: Adversarial review of story-1-5 identified the gap; the spec itself acknowledges the last-holder guard but does not address concurrency. Explicitly carved off because Story 1.5's bounded scope covers the single-threaded path; fixing the race requires either a DB-level constraint or a serializable transaction wrapping both the check and the mutation, neither of which is representable within `IFullProfileAccessRepository`'s current interface shape without redesign. Not a concern in the current single-node deployment but must be addressed before horizontal scaling or high-traffic grant/revoke is expected.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: `FullProfileAccessAction` enum is stored as an integer by EF Core's default convention (0=Grant, 1=Revoke). If the enum definition is ever reordered or extended, existing journal rows become misinterpreted. Add `.HasConversion<string>()` in `AccessControlDbContext.OnModelCreating` to store values as stable, human-readable strings (`"Grant"`, `"Revoke"`), matching the pattern used for `ProjectAssignmentRole` in this service.
  evidence: Edge-case review of story-1-5 found no explicit `HasConversion` on `FullProfileAccessJournalEntry.Action`. Deferred because it requires a new migration (change column type from int to text + backfill the two seeded rows), not just a model change; the table has only two rows at this point, so the backfill is trivial, but the migration itself needs to land cleanly with the existing data.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: `FullProfileAccessTests` joins the `[Collection("HealthEndpointTests")]` shared collection and uses a Testcontainers Postgres container scoped to the test class. Tests in this class mutate `full_profile_access_grants` (grant/revoke sequences) — if xUnit ever runs two tests from this class concurrently against the same container instance, the last-holder guard and count assertions may race. Validate that the class-level `IAsyncLifetime` pattern (one container per test class, not per test) actually provides adequate isolation given the mutation pattern, and add `[Collection]`-level or test-method-level ordering annotations if needed.
  evidence: Edge-case review of story-1-5 flagged the shared-state concern; the current `IAsyncLifetime`/`[Collection]` pattern was inherited from `AccessRoleResolverCompositionTests` and is consistent with the rest of this test project, but `FullProfileAccessTests` is the first class in the project whose tests *mutate* the grants table rather than only reading from it. No failure observed in the current serial test run; the concern is latent under future parallelization.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: No test covers the self-view scenario: a Full-profile-access holder resolving their own profile (`viewerPersonId == subjectPersonId`). The resolver and profile assembly both allow this path, but no test asserts the expected behavior (all 16 sections RW, since `fullProfileAccessLine=true` takes precedence). Add a test for `GET /api/v1/access-roles/resolve?viewerPersonId={PlatformLeadId}&subjectPersonId={PlatformLeadId}` asserting `fullProfileAccessLine=true` and the full-access section group returned.
  evidence: Verification-gap review of story-1-5 found this case absent from both `FullProfileAccessTests` and `AccessRoleResolverCompositionTests`; the I/O matrix (spec §3) does not list self-view as a named scenario either, leaving it implicitly covered by the general "Resolve — holder" row without a concrete test pinning it.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: `people-service`'s `fullProfileAccessSectionAccess` type only declares five sections (s1, s2, s10, s11, s16) — the sections people-service currently assembles — rather than all 16. If people-service extends profile assembly to additional sections (S3–S9, S12–S15), each new section must also be added to this type and threaded through `resolveAudience`. There is no compile-time enforcement that the type stays in sync with the assembled sections.
  evidence: Verification-gap review of story-1-5 flagged the partial type. Intentional at this story's scope (people-service only assembles five sections today), but it creates a maintenance trap: adding a section to profile assembly without adding it to the FPA type would silently give FPA holders the same access as non-holders on that new section. Recommend adding a TODO comment in `profile.ports.ts` next to the type definition, or a runtime assertion, as a canary for future implementers.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-5-full-profile-access-as-a-separate-journaled-grant.md`
  summary: `FullProfileAccessController`'s XML doc comment references `IFullProfileAccessRepository` by its full type name in the `<see>` tag, coupling the Api-layer doc to the Domain interface type — consistent with other controllers in this service, but worth revisiting if the controller doc is ever extracted to a public contract or published API spec, where infrastructure/domain type names should not appear.
  evidence: Minor style finding from edge-case review of story-1-5; no functional impact, not actionable before the service has a published external API contract.
