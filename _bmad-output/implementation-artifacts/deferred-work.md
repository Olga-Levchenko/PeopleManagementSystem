...
- source_spec: `_bmad-output/implementation-artifacts/spec-1-8-colleague-view-field-whitelist.md`
  summary: Add runtime validation (or a Zod/class-validator schema) to `HttpAccessRoleResolutionAdapter.resolve`'s `response.json() as AccessRoleResolution` cast so that wire-shape drift from access-control-service (unexpected key casing, removed fields, schema evolution) produces a structured error rather than silently degrading to `undefined` reads that fail closed by coincidence.
  evidence: Edge-case-hunter review of story-1-8 found the adapter relies on fail-closed behavior being an emergent property of `undefined`-is-falsy semantics rather than an explicit invariant; the cast is safe today because every failure mode is falsy, but a future AC-service field change that is truthy-but-wrong (e.g. a renamed boolean `reportingLineActive` instead of `reportingLine`) would grant access incorrectly rather than failing closed. Deferred because adding a runtime schema validator is a cross-cutting concern affecting all AC-service consumers, not just Story 1.8.

- source_spec: `_bmad-output/implementation-artifacts/spec-1-8-colleague-view-field-whitelist.md`
  summary: Decide whether `Leave.leaveType` should be made non-nullable (all leaves must have a type) or explicitly documented as nullable with a meaning (e.g. "type unknown at import time") — the schema currently permits a leave entry with no type, which `toS10`'s full mapper silently omits from the response, producing an entry with `startDate`/`endDate` only even for non-colleague audiences where type visibility is expected.
  evidence: Edge-case-hunter review of story-1-8 noted the nullable column has no DB constraint or application-layer guard preventing empty/null leaveType from silently producing an under-populated response for manager/self viewers; the spec's S10 payload description expects a type to be present. Deferred because backfilling or constraining this requires a product decision on what an absent type means (data integrity vs. schema flexibility).

- source_spec: `_bmad-output/implementation-artifacts/spec-1-8-colleague-view-field-whitelist.md`
  summary: Add ordering (`orderBy`) and optionally pagination (`take`/`skip`) to `people-service`'s Prisma `leaves` and `personProjectAssignments` selects in `ProfileService.getProfile` before real org-scale data exists, to ensure deterministic ordering in e2e tests and avoid unbounded memory loading for subjects with large leave/assignment histories.
  evidence: Edge-case-hunter review of story-1-8 found no `orderBy` on either relation select — row order is non-deterministic (Postgres makes no ordering guarantee without ORDER BY), making e2e tests order-dependent only by coincidence of single-row seeding, and a subject with hundreds of leaves would load all rows into memory in one query. Deferred because no ordering or volume requirement is spec'd for S10/S11 in this story; revisit when the real data model and UX pagination requirements for these sections are defined.
