...
## Deferred from: code review of spec-5-1-shared-dashboard-engine-per-audience-configuration (2026-09-14)

- source_spec: `_bmad-output/implementation-artifacts/spec-5-1-shared-dashboard-engine-per-audience-configuration.md`
  summary: `DashboardTable` renders an empty `<table>` with no header or body rows when `columns` is an empty array and `emptyMessage` is absent — invalid table semantics per the HTML spec (a `<table>` must have at least one column). TypeScript's required `columns: ColumnSpec<T>[]` prop prevents this at compile time for typed callers, but a runtime-only call (e.g. from a test or an untyped context) could still trigger it. Add a guard or document the invariant.
  evidence: Code review of Story 5.1 edge-case pass; zero blast radius today since all callers are typed — TypeScript enforces a non-empty config array in practice. Deferred: adding a runtime guard (`columns.length === 0`) would require deciding on a fallback render, which is scope-expanding relative to the extraction objective.

- source_spec: `_bmad-output/implementation-artifacts/spec-5-1-shared-dashboard-engine-per-audience-configuration.md`
  summary: `DashboardTable` emits `aria-sort="none"` statically on sortable column headers regardless of actual sort state — sorting is not wired in Story 5.1 (no sort state, no sort dispatch). The attribute accurately signals "this column is sortable" per the current codebase state but will become misleading once a caller adds real sort state: whichever column is active should flip to `"ascending"` or `"descending"`, and all others should flip to `"none"`. The table currently provides no `onSortChange` prop or sort-direction input; a future caller that needs real aria-sort must add both.
  evidence: Code review of Story 5.1 edge-case pass; `aria-sort` management was explicitly not in this story's scope (`DashboardTable`'s spec requires only that sortable headers carry the attribute — not that sort state be tracked). Deferred: sort state management belongs to the story that first adds a sortable, interactive dashboard column.
